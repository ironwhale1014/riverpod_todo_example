...
class TodoService extends _$TodoService {
  ...
  Future<void> saveTodo({required String description, int? category}) async {
  ...
  Future<List<TodoWithCategory>> search(String text) async {
    return appDatabase
        .search(text)
        .map((row) => TodoWithCategory(todoEntry: row.todos, category: row.cat))
        .get();
  }
}
